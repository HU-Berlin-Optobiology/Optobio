...
macro "Copy to System [c]" {
	run("Copy to System");
	showMessage("Copied to system");
}

macro "Save as Tiff [s]" {
	Name = getTitle();
	SaveDir = getDirectory("SaveDir");
	saveAs("Tiff", SaveDir + "/"+ Name);
	close(Name);
}
macro "Make CLAHE [e]" {
	run("Enhance Local Contrast (CLAHE)", "blocksize=50 histogram=256 maximum=3 mask=*None*");
	waitForUser("CLAHE finished", "CLAHE has been finished. Click OK to continue");
}
macro "Delete seleted ROI inside Manager [Y]" {
	roiManager("delete");
}
macro "Run Auto Contrast [y]" {
	run("Enhance Contrast", "saturated=0.35");
}
macro "Safe ROIs as .zip in designated path [S]" {
	dir = getDirectory("Path for ROI to be saved");
	name = getString("Enter name of to be saved ROIs", "DefaultROIName");
	roiManager("save", dir+"/"+name+".zip");
}
macro "Change state of channel 1 [1]" {
	Stack.getActiveChannels(Ch);
	Ch1 = substring(Ch, 0, 1);
	Ch2 = substring(Ch, 1, 2);
	Ch3 = substring(Ch, 2, 3);
	Ch4 = substring(Ch, 3, 4);
	if (Ch1 == "0") {
		Ch1 = "1";
	}
	else if (Ch1 == "1") {
	...
	Stack.setActiveChannels(Ch1+Ch2+Ch3+Ch4);
}
macro "Change state of channel 2 [2]" {
	Stack.getActiveChannels(Ch);
	Ch1 = substring(Ch, 0, 1);
	Ch2 = substring(Ch, 1, 2);
	Ch3 = substring(Ch, 2, 3);
...
macro "Change state of channel 3 [3]" {
	Stack.getActiveChannels(Ch);
	Ch1 = substring(Ch, 0, 1);
	Ch2 = substring(Ch, 1, 2);
	Ch3 = substring(Ch, 2, 3);
...
macro "Change state of channel 4 [4]" {
	Stack.getActiveChannels(Ch);
	Ch1 = substring(Ch, 0, 1);
	Ch2 = substring(Ch, 1, 2);
	Ch3 = substring(Ch, 2, 3);
